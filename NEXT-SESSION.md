...
| Overview refresh (v31.2) | header logo = animated orbital emblem (gradient ring + counter-rotating satellites + pulsing core, SVG w/ theme vars) + full-gradient shimmer wordmark (`.logo-title`); Panel titles get a page-accent tick; **Ops Pulse** = glow tiles w/ live queue dots, 12-bucket today sparkline, integrity ring gauge (lime/amber/rose by ratio); Claude Mission Totals = same glow tiles; Needs Attention all-clear = pulse-ring check emblem "ALL SYSTEMS NOMINAL", items get orbs + accent wash. All colors via `--ac-*`/ACCENTS (both themes verified) |
