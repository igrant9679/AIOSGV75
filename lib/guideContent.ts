...
- **GLM (Z.ai) — same trap, same shape.** Z.ai has **two OpenAI-compatible endpoints and they are NOT interchangeable**. *GLM Coding Plan (subscription)* = \`https://api.z.ai/api/coding/paas/v4\`, model \`glm-5.2\`, key from the **Coding Plan console** — flat monthly fee, so Auto ranks it cheapest (rank 1). *GLM (Z.ai) — pay per token* = \`https://api.z.ai/api/paas/v4\`, billed ~$1.40/$4.40 per M. Pointing a Coding Plan key at the general endpoint bills you per token while your subscription sits unused. The plan is **prompt-quota based, not unlimited** (Lite ≈ 80 prompts/5h) and GLM-5.2 burns **3× quota during 14:00–18:00 UTC+8** — a great default agent, a poor bulk worker. ⚠ Hermes's built-in \`zai\` provider also hardcodes the metered \`/api/paas/v4\`, so wire the subscription here in Settings rather than in Hermes.
