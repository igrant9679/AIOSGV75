...
**Control Room says "Hermes isn't installed"** — that message means \`HERMES_BIN\` isn't set in this machine's \`.env.local\` (the app never scans the disk). Two traps behind it: (1) the path must use THIS machine's Windows username — check with \`echo $env:USERNAME\` — not a path copied from another machine; (2) env is read only at boot, so restart the server after editing. And on a **fresh Hermes install**, port 9119 stays dead even with \`HERMES_BIN\` correct: the supervisor's \`--skip-build\` flag serves a pre-built UI that doesn't exist yet. Build it once by hand — \`hermes dashboard --no-open\` in a terminal (a few minutes; looks hung) — then Ctrl+C once it's listening on 9119; auto-start works from then on.
